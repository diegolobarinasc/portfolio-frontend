import "./CV.css";
 
export default function CV() {
  return (
      <div className="cv-container">
 
        {/* SIDEBAR */}
        <aside className="cv-sidebar">
          <div className="cv-photo-wrapper">
            <img src="/foto_cv.png" alt="Diego Miguel Lobariñas Criserá" className="cv-photo" />
          </div>
 
          <div className="cv-sidebar-block">
            <h3 className="cv-sidebar-title">CONTACTO</h3>
            <ul className="cv-contact-list">
              <li>
                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="#a0b3c1">
                  <path d="M12 12.713l-11.985-9.713h23.97l-11.985 9.713zm0 2.574l-12-9.725v15.438h24v-15.438l-12 9.725z"/>
                </svg>
                diegolobarinasc@gmail.com
              </li>
              <li>
                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="#a0b3c1">
                  <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/>
                </svg>
                <a href="https://www.linkedin.com/in/diego-lobariñas-crisera" target="_blank" rel="noreferrer">
                  linkedin.com/in/diego-lobariñas-crisera
                </a>
              </li>
              <li>
                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="#a0b3c1">
                  <path d="M12 0c-6.627 0-12 5.373-12 12s5.373 12 12 12 12-5.373 12-12-5.373-12-12-12zm1 16.057v-3.057h2.994c-.059 1.143-.212 2.24-.456 3.279-.823-.12-1.674-.188-2.538-.222zm1.957 2.162c-.499 1.33-1.159 2.497-1.957 3.456v-3.62c.666.028 1.319.081 1.957.164zm-1.957-7.219v-3.015c.868-.034 1.721-.103 2.548-.224.238 1.027.389 2.111.446 3.239h-2.994zm0-5.014v-3.661c.806.969 1.471 2.15 1.971 3.496-.642.084-1.3.137-1.971.165zm2.703-3.267c1.237.496 2.354 1.228 3.29 2.146-.642.234-1.311.442-2.019.607-.344-.992-.775-1.91-1.271-2.753zm-7.241 13.56c-.244-1.039-.398-2.136-.456-3.279h2.994v3.057c-.865.034-1.714.102-2.538.222zm2.538 1.776v3.62c-.798-.959-1.458-2.126-1.957-3.456.638-.083 1.291-.136 1.957-.164zm-2.994-7.055c.057-1.128.207-2.212.446-3.239.827.121 1.68.19 2.548.224v3.015h-2.994zm1.024-5.179c.5-1.346 1.165-2.527 1.97-3.496v3.661c-.671-.028-1.329-.081-1.97-.165zm-2.005-.35c-.708-.165-1.377-.373-2.018-.607.937-.918 2.053-1.65 3.29-2.146-.496.843-.927 1.761-1.272 2.753zm-.549 1.918c-.264 1.151-.434 2.36-.492 3.611h-3.933c.165-1.658.739-3.197 1.617-4.518.88.361 1.816.67 2.808.907zm.009 9.262c-.988.236-1.92.542-2.797.9-.89-1.328-1.471-2.879-1.637-4.551h3.934c.058 1.265.231 2.488.5 3.651zm.553 1.917c.342.976.768 1.881 1.257 2.712-1.223-.49-2.326-1.211-3.256-2.115.636-.229 1.299-.435 1.999-.597zm9.924 0c.7.163 1.362.367 1.999.597-.931.903-2.034 1.625-3.257 2.116.489-.832.915-1.737 1.258-2.713zm.553-1.917c.27-1.163.442-2.386.5-3.651h3.934c-.167 1.672-.748 3.223-1.638 4.551-.877-.358-1.81-.664-2.796-.9zm.501-5.651c-.058-1.251-.229-2.46-.492-3.611.992-.237 1.929-.546 2.809-.907.877 1.321 1.451 2.86 1.616 4.518h-3.933z"/>
                </svg>
                <a href="https://dmlc-portfolio.vercel.app/" target="_blank" rel="noreferrer">
                  Portfolio
                </a>
              </li>
            </ul>
          </div>
 
          <div className="cv-sidebar-block">
            <h3 className="cv-sidebar-title">HABILIDADES TÉCNICAS</h3>
            <ul className="cv-skills-list">
              <li><strong>IA Conversacional</strong>: Dialogflow CX, IBM Watson Assistant</li>
              <li><strong>NLP aplicado</strong>: intents, entities, training phrases</li>
              <li><strong>Diseño Conversacional</strong>: flujos, UX conversacional, gestión de errores</li>
              <li><strong>Lenguajes</strong>: JavaScript, Python, HTML, VBA, Twig</li>
              <li><strong>Datos y Configuración</strong>: JSON</li>
              <li><strong>Automatización</strong>: UiPath</li>
              <li><strong>Herramientas</strong>: Postman, Jira, Confluence, Microsoft Excel</li>
            </ul>
          </div>
 
          <div className="cv-sidebar-block">
            <h3 className="cv-sidebar-title">HABILIDADES PROFESIONALES</h3>
            <ul className="cv-skills-list">
              <li>Trabajo en equipo</li>
              <li>Análisis y resolución de problemas</li>
              <li>Planificación</li>
              <li>Liderazgo</li>
              <li>Orientación a resultados</li>
            </ul>
          </div>
 
          <div className="cv-sidebar-block">
            <h3 className="cv-sidebar-title">IDIOMAS</h3>
            <p className="cv-sidebar-text">
              Español y gallego nativos, inglés Intermedio (buena comprensión lectora y auditiva)
            </p>
          </div>
 
          <a href="/CV_DiegoLobarinasCrisera.pdf" download className="cv-download-btn">
            ↓ Descargar CV en PDF
          </a>
        </aside>
 
        {/* MAIN */}
        <main className="cv-main">
 
          <div className="cv-header-block">
            <h1 className="cv-name">DIEGO MIGUEL LOBARIÑAS CRISERÁ</h1>
            <h2 className="cv-subtitle">CONVERSATIONAL AI SPECIALIST (DIALOGFLOW CX / IBM WATSON)</h2>
            <p className="cv-summary">
              Especialista en <strong>IA conversacional</strong> con amplia experiencia en el diseño,
              desarrollo y optimización de chatbots empresariales para atención al cliente y automatización
              de procesos. Especializado en <strong>Google Dialogflow CX</strong> e{" "}
              <strong>IBM Watson Assistant</strong>, centrada en el <strong>diseño conversacional</strong>,
              definición y optimización de <strong>intents y entidades</strong>, y mejora continua basada en{" "}
              <strong>análisis de logs, métricas de rendimiento y técnicas de NLP aplicadas mediante
              plataforma</strong>. Perfil técnico con background en automatización, análisis de datos y coordinación en proyectos con múltiples actores.
            </p>
          </div>
 
          <div className="cv-section-block">
            <h2 className="cv-section-title">EXPERIENCIA</h2>
 
            <div className="cv-job">
              <div className="cv-job-header">
                <span className="cv-job-dot" />
                <span className="cv-job-period"><strong>2021 - Actualidad</strong> | Jazzplat España</span>
              </div>
              <h3 className="cv-job-title">Conversational IA Specialist</h3>
              <ul className="cv-job-bullets">
                <li>Diseño, implementación y optimización de <strong>chatbots empresariales</strong> sobre <strong>Dialogflow CX</strong> e <strong>IBM Watson Assistant</strong> en entornos productivos: definición de <strong>intents</strong>, <strong>entidades</strong> y <strong>flujos conversacionales</strong> aplicando <strong>NLPa nivel de plataforma</strong> .</li>
                <li>Rápida adaptación a nuevas herramientas y plataformas, aplicando conocimientos a proyectos productivos.</li>
                <li>Análisis de logs y métricas para <strong>mejora continua del rendimiento y UX conversacional</strong>.</li>
                <li>Referente técnico de Dialogflow CX en el proyecto, con interlocución directa con el cliente, otras consultoras y diseñadores de diálogo para trasladar capacidades de la plataforma a soluciones funcionales con buena UX.</li>
                <li>Asesoramiento al cliente en la planificación de sprints: definición de tareas por iteración y creación de tickets en Jira.</li>
              </ul>
            </div>
 
            <div className="cv-job">
              <div className="cv-job-header">
                <span className="cv-job-dot" />
                <span className="cv-job-period"><strong>2018 - 2021</strong> | Jazzplat España</span>
              </div>
              <h3 className="cv-job-title">Coordinador de Planificación</h3>
              <ul className="cv-job-bullets">
                <li>Planificación operativa y <strong>dimensionamiento de personal (Erlang)</strong> para cumplimiento de KPIs.</li>
                <li><strong>Coordinación de equipos multidisciplinares</strong> y gestión operativa en tiempo real, asegurando alineación entre objetivos de negocio y capacidades del equipo.</li>
                <li>Supervisión de equipos y gestión operativa en tiempo real.</li>
              </ul>
            </div>
 
            <div className="cv-job">
              <div className="cv-job-header">
                <span className="cv-job-dot" />
                <span className="cv-job-period"><strong>2016 - 2018</strong> | SERMICRO</span>
              </div>
              <h3 className="cv-job-title">Analista</h3>
              <ul className="cv-job-bullets">
                <li>Supervisión de plataformas Jazztel/Orange y <strong>automatización de reporting operativo</strong>.</li>
              </ul>
            </div>
 
            <div className="cv-job">
              <div className="cv-job-header">
                <span className="cv-job-dot" />
                <span className="cv-job-period"><strong>2013 - 2016</strong> | Jazzplat España</span>
              </div>
              <h3 className="cv-job-title">Analista</h3>
              <ul className="cv-job-bullets">
                <li>Monitorización de plataformas y análisis de incidencias operativas de plataformas Jazztel.</li>
              </ul>
            </div>
 
            <div className="cv-job">
              <div className="cv-job-header">
                <span className="cv-job-dot" />
                <span className="cv-job-period"><strong>2008 - 2013</strong> | Unísono Soluciones de Negocio</span>
              </div>
              <h3 className="cv-job-title">Coordinador/Formador</h3>
              <ul className="cv-job-bullets">
                <li>Coordinación y formación de equipos en campañas de Orange.</li>
              </ul>
            </div>
          </div>
 
          <div className="cv-section-block">
            <h2 className="cv-section-title">DATOS ACADÉMICOS</h2>
            <h3 className="cv-job-title">Universidade de Vigo</h3>
            <p className="cv-academic-text">Diplomatura en Ingeniería Industrial</p>
          </div>
 
        </main>
      </div>
  );
}